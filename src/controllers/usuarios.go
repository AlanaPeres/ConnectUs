...
func BuscarUsuarios(w http.ResponseWriter, r *http.Request) {
	...
	nomeOuNick := strings.ToLower(r.URL.Query().Get("usuario")) //traz tudo oq está na query da nossa rota, podendo ter mais de um valor
	db, erro := banco.Conectar()
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
...
func DeixarDeSeguir(w http.ResponseWriter, r *http.Request) {
	...
	if seguidorID == usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não é possível parar de seguir a sí mesmo"))
		return
	}
	db, erro := banco.Conectar()
	if erro != nil {
...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	usuarioNoToken, erro := autenticacao.ExtrairUsuarioID(r)
	if erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, erro)
		return
	}
	...
	if usuarioNoToken != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não é possível alterar uma senha de um usuário que não é o seu."))
		return
	}
